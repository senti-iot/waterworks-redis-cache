...
export const execCronBenchmark = async (uuids, orgId, period) => {
	if (period) {
		...
		let waterTemp = await Promise.all(timeTables.map(async t => ({ p: t, data: await getBenchmark(orgId, uuids, t) })))
		storeData(uuids, orgId, waterTemp, 'benchmark')
	}
}
...
export const execCronWaterTemperature = async (uuids, orgId, period) => {
	if (period) {
		...
		let waterTemp = await Promise.all(timeTables.map(async t => ({ p: t, data: await getWaterTemperature(orgId, uuids, t) })))
...
export const execCronAmbientTemperature = async (uuids, orgId, period) => {
	if (period) {
		...
		let ambientTemp = await Promise.all(timeTables.map(async t => ({ p: t, data: await getAmbientTemperature(orgId, uuids, t) })))
...
export const execCronMaxFlow = async (uuids, orgId, period) => {
	if (period) {
		...
		let maxFlow = await Promise.all(timeTables.map(async t => ({ p: t, data: await getMaxFlow(orgId, uuids, t) })))
...
export const execCronMinFlow = async (uuids, orgId, period) => {
	if (period) {
		...
		let minFlow = await Promise.all(timeTables.map(async t => ({ p: t, data: await getMinFlow(orgId, uuids, t) })))
...
export const execCronReading = async (uuids, orgId, period) => {
	if (period) {
		...
		let reading = await Promise.all(timeTables.map(async t => ({ p: t, data: await getReading(orgId, uuids, t) })))
